async function lab_graph(start_num_nodes, end_num_nodes, pro_edges, directed, epoch, algorithms) {
  ...
  for (var num_nodes = start_num_nodes; num_nodes <= end_num_nodes; num_nodes++) {
    ...
    for(var i = 0; i < epoch; i++) {
      var graph = await load_graph_and_incremental_edge(num_nodes, pro_edges, directed);
      ...
      for(var algorithm_index = 0; algorithm_index < algorithms.length; algorithm_index++) {
        ...
        if (algorithm.name == 'rr-bfs-truncated')
          var dist = await run_algoritm_rr_bfs_truncated(graph.values);
